document.addEventListener('DOMContentLoaded', () => {
    ...
    // ScrollSpy para as Abas da Navbar
    ...
    const revealElements = document.querySelectorAll('.portfolio-item, .skills-list, .skills-software, .sobre-text, .sobre-image, .section-title, .section-script, .process-card, .stats-grid');
        ...
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const updateCount = () => {
                    ...
                    const increment = target / 30; // Controla a velocidade
    ...
    const clickableElements = document.querySelectorAll('a, button, .filter-btn, .close-modal, .play-overlay');
    ...
    // Efeito Netflix e Cursor Inteligente
    portfolioItems.forEach(item => {
        ...
        item.addEventListener('mouseleave', () => {
            if (cursor) {
                cursor.classList.remove('cursor-play', 'cursor-view');
                cursor.innerText = "";
            }
        });
    });
